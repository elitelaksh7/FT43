...
try:
    ...
    app = FastAPI(title="WalletWise OCR API")
    ...
    def extract_payment_app_transactions(text, image=None):
        try:
            ...
            if not transactions and "test" in text.lower():
                transactions = [
                    {
                        "merchant": "EatClub",
                        "amount": 220,
    ...
    @app.get("/")
    def root():
        return {"message": "WalletWise OCR API is running!", "endpoints": ["/parse-image/", "/parse-payment-app/"]}
