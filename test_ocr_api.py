#!/usr/bin/env python
"""
Test script for the OCR API - Specifically for payment app screenshots
"""
import requests
import json
import os
from pprint import pprint

def test_ocr_payment_app_api():
    """
    Test the OCR API by sending a sample payment app screenshot
    """
    print("Testing OCR API for payment app screenshots...")
    
    # Path to a sample payment app screenshot (replace with your actual file path)
    image_path = "test_payment_screenshot.jpg"
    
    if not os.path.exists(image_path):
        print(f"Warning: Test image not found at {image_path}")
        print("Creating a dummy test file...")
        # Create a dummy file for testing
        with open(image_path, "wb") as f:
            # Write a minimal valid JPEG file
            f.write(bytes([
                0xFF, 0xD8,  # SOI
                0xFF, 0xE0, 0x00, 0x10, 0x4A, 0x46, 0x49, 0x46, 0x00, 0x01, 0x01, 0x01, 0x00, 0x48, 0x00, 0x48, 0x00, 0x00,  # APP0
                0xFF, 0xDB, 0x00, 0x43, 0x00, 0x08, 0x06, 0x06, 0x07, 0x06, 0x05, 0x08, 0x07, 0x07, 0x07, 0x09, 0x09, 0x08, 0x0A, 0x0C, 0x14, 0x0D, 0x0C, 0x0B, 0x0B, 0x0C, 0x19, 0x12, 0x13, 0x0F, 0x14, 0x1D, 0x1A, 0x1F, 0x1E, 0x1D, 0x1A, 0x1C, 0x1C, 0x20, 0x24, 0x2E, 0x27, 0x20, 0x22, 0x2C, 0x23, 0x1C, 0x1C, 0x28, 0x37, 0x29, 0x2C, 0x30, 0x31, 0x34, 0x34, 0x34, 0x1F, 0x27, 0x39, 0x3D, 0x38, 0x32, 0x3C, 0x2E, 0x33, 0x34, 0x32,  # DQT
                0xFF, 0xC0, 0x00, 0x11, 0x08, 0x00, 0x01, 0x00, 0x01, 0x03, 0x01, 0x11, 0x00, 0x02, 0x11, 0x01, 0x03, 0x11, 0x01,  # SOF0
                0xFF, 0xC4, 0x00, 0x1F, 0x00, 0x00, 0x01, 0x05, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B,  # DHT
                ,  # DHT
                0xFF, 0xDA, 0x00, 0x0C, 0x03, 0x01, 0x00, 0x02, 0x11, 0x03, 0x11, 0x00, 0x3F, 0x00,  # SOS
                0x00,  # Image data
                0xFF, 0xD9  # EOI
            ]))

    try:
        # API endpoint
        url = "http://localhost:8002/parse-payment-app/"
        
        # Send a POST request with the image file
        with open(image_path, "rb") as img_file:
            files = {"file": (os.path.basename(image_path), img_file, "image/jpeg")}
            response = requests.post(url, files=files)
            
        # Check response status
        if response.status_code == 200:
            print("API call successful!")
            result = response.json()
            print("\nAPI Response:")
            pprint(result)
            
            if result.get("transactions", []):
                print(f"\nFound {len(result['transactions'])} transactions")
                for i, tx in enumerate(result["transactions"], 1):
                    print(f"Transaction {i}:")
                    print(f"  Merchant: {tx.get('merchant')}")
                    print(f"  Amount: {tx.get('amount')}")
                    print(f"  Date: {tx.get('date')}")
                    print(f"  Type: {tx.get('type')}")
                    print("-" * 30)
            else:
                print("No transactions found in the image")
                
            return result
        else:
            print(f"API call failed with status code: {response.status_code}")
            print("Response content:", response.text)
            return None
    except Exception as e:
        print(f"Error testing API: {str(e)}")
        return None

def test_frontend_api_call():
    """
    Test the frontend API call to the OCR service
    """
    print("\nTesting frontend API call to the OCR service...")
    
    # Create a mock function that simulates the frontend fetch call
    def mock_frontend_fetch():
        url = "http://localhost:8002/parse-payment-app/"
        image_path = "test_payment_screenshot.jpg"
        
        if not os.path.exists(image_path):
            print(f"Warning: Test image not found at {image_path}")
            return False
        
        try:
            # Create form data similar to how frontend would do it
            with open(image_path, "rb") as img_file:
                files = {"file": (os.path.basename(image_path), img_file, "image/jpeg")}
                response = requests.post(url, files=files)
                
            if response.status_code == 200:
                print("Frontend API call would succeed!")
                return True
            else:
                print(f"Frontend API call would fail with status: {response.status_code}")
                return False
        except Exception as e:
            print(f"Error in frontend API call: {str(e)}")
            return False
    
    return mock_frontend_fetch()

if __name__ == "__main__":
    print("=== OCR API Test Script ===")
    test_ocr_payment_app_api()
    test_frontend_api_call()
    print("\nTest script completed.")