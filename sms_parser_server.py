#!/usr/bin/env python3
"""
SMS Transaction Parser Server - A FastAPI server to parse SMS transaction data.
"""
...
app = FastAPI(
    ...
    description="API for parsing SMS transaction text and extracting structured data."
)
...
@app.post("/parse-sms")
def parse_sms_transaction(sms: SMSText) -> ParsedTransaction:
    """
    ...
    try:
        transaction_details = extract_transaction_details(sms.text)
...
if __name__ == "__main__":
    ...
    # More compatible way to run the server
    import uvicorn
    uvicorn.run("sms_parser_server:app", host="0.0.0.0", port=8002, log_level="info")